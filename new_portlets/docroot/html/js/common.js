...
function renderPaginator_new(data, divId) {
	...
	if (data.items.length > 0) {
		...
		var currentPageIndex = data.currentPageNum - 3;
		if (currentPageIndex > data.totalPageNum - 5) {
			currentPageIndex = data.totalPageNum - 5;
		}
		...
		console.log("Total page is: " + data.totalPageNum);
		for (var i = currentPageIndex; i < data.totalPageNum; i++) {
			data.pages.push({
				index: i + 1
			});
			
			if (i - currentPageIndex < 5) {
				data.displayPages.push({
					index: i + 1
				});
			} 
			
			if ((i + 1) == data.currentPageNum) {
				data.displayPages[i-currentPageIndex].active = true;
			}
		}
...
var TEMPLATES_FILE = {
	...
	,
	news_display_type_audio : {
		location : MAIN_TEMPLATE_FILE,
		id : "news_display_type_audio"
	}
	,
	portlet_audio_player : {
		location : MAIN_TEMPLATE_FILE,
		id : "portlet_audio_player"
	}
...
var DISPLAY_TYPE_TEMPLATE_NAME = {
	...
	"portlet.news.config.display-type.audio" : "news_display_type_audio" ,
	"portlet.audio.player" : "portlet_audio_player"
};
...
function renderListByCategoryPage(data, cssSelector) {
    ...
    if(data.displayType == 'portlet.news.config.display-type.audio') {
		for(var k=0 ; k < data.items.length ; k++) {
			data.items[k].downloadUrl =document.location.origin + data.items[k].otherProps['media'];
		}
	}
    if(data.displayType == 'portlet.news.config.display-type.visitor') {
		data.first = data.items[0];
		data.calendar = getDateTimeText(data.items[0].otherProps['Ngày diễn ra sự kiện']);
		data.items = data.items.splice(1,data.items.length);
	}
...
function renderListAudio(data, cssSelector) {
	loadTemplate(DISPLAY_TYPE_TEMPLATE_NAME["portlet.audio.player"], function(
			htmlTemplate) {
		$(cssSelector).html(Mustache.render(htmlTemplate, data));
		renderPlayer();
	});
...
function sortJSONEvent(items, key, way) {
    return items.sort(function(a, b) {
        var x = a.otherProps[key]; var y = b.otherProps[key];
        if (way === '123' ) { return ((compareDates(y,x)) ? -1 : ((compareDates(x,y)) ? 1 : 0)); }
...
function loadCategoryPageTab(categoryId, tabContentId, URL, includePaging) {
	...
	$.ajax({
		...
		datatype : 'json',
		success : function(data) {
			console.log(" tabContentId ___ " + tabContentId);
			console.log(" divid  ____ " + categoryId);
			console.log(data);
...
function getDateTimeText(strDate) {
	var jsMonths = [];
	jsMonths.push({
		months: $('#months_' + strDate.split('-')[1]).val() ,
