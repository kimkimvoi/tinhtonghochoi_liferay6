...
public class NewsUtils {
	...
	public static String getURL(PortletRequest request, JournalArticle article, String friendlyURL, String portletName, String structId,long categoryId) {
...
//			logger.error(e.getMessage());
	...
	public static String getAudioDetailUrl(PortletRequest request, DLFileEntry fileEntry, String friendlyURL, String portletName, String structureId) {
		PortletURL portletURL = PortletURLFactoryUtil.create(request, portletName, 
				PortalUtil.getLayoutIdViewDetail(request, friendlyURL), PortletRequest.RENDER_PHASE);
		try {
			portletURL.setWindowState(WindowState.NORMAL);
			portletURL.setPortletMode(PortletMode.VIEW);
		} catch (Exception e) {
		...
		portletURL.setParameter("audioTitle", FriendlyURLNormalizerUtil.normalize(fileEntry.getTitle().split(".mp3")[0]).replace(".", "-"));
		portletURL.setParameter("fileEntryId", String.valueOf(fileEntry.getFileEntryId()));
		
		return portletURL.toString();
	}
	
	public static String getUrlByCategory(PortletRequest request, DLFolder folder, String friendlyUrl, String portletName) {
		PortletURL portletURL = PortletURLFactoryUtil.create(request, portletName, 
				PortalUtil.getLayoutIdViewDetail(request, friendlyUrl), PortletRequest.RENDER_PHASE);
		try {
			portletURL.setWindowState(WindowState.NORMAL);
		...
		portletURL.setParameter("categoryId", String.valueOf(folder.getFolderId()));
		
		return portletURL.toString();
	}
	
	public static String getAudioDetailUrl(PortletRequest request, DLFileEntry fileEntry, String friendlyURL, String portletName, String structureId, String urlAuthor) {
		PortletURL portletURL = PortletURLFactoryUtil.create(request, portletName, 
				PortalUtil.getLayoutIdViewDetail(request, friendlyURL), PortletRequest.RENDER_PHASE);
		try {
			portletURL.setWindowState(WindowState.NORMAL);
		...
		portletURL.setParameter("fileEntryId", String.valueOf(fileEntry.getFileEntryId()));
		portletURL.setParameter("urlAuthor", urlAuthor);
		
		return portletURL.toString();
	}
	/**
	 * This method using get url
	 * @param request
	 * @param article
	 * @param friendlyURL
	 * @param portletName
	...
	public static String getURL(PortletRequest request, JournalArticle article, String friendlyURL, String portletName, String structId) {
		...
		try {
			...
			portletURL.setWindowState(WindowState.NORMAL);
			portletURL.setPortletMode(PortletMode.VIEW);
			portletURL.setParameter("urlTitle", article.getUrlTitle());
	...
	public static boolean isNewNews(String date) {
		...
		try {
			...
			if(crDate.compareTo(oneMonthAgo) > 0) return true;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return false;
