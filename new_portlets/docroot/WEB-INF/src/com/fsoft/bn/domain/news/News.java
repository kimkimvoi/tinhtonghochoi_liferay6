...
public class News {
	...
	public News(JournalArticle article, String urlDetail, String languageId) {
		...
		setStrucId(article.getStructureId());
		...
		for (; it.hasNext();) {
			...
			if (key.equalsIgnoreCase(XSD_ELEMENT_NAME.CONTENT)) {
				...
				it.remove();
			} else if (key.equalsIgnoreCase(XSD_ELEMENT_NAME.VOTE_QUESTION)) {
				setVoteQuestion(entry.getValue());
				it.remove();
			}
		}
	}
	public News(JournalArticle article, String urlDetail, String urlDetailAudio, String languageId) {
		//set value to this news
		...
		setImageURL(article.getSmallImageURL());
		setUrlDetail(urlDetail);
		setUrlDetailAudio(urlDetailAudio);
		setViewCount(NewsUtils.getViewCountArticle(article.getResourcePrimKey()));
		//check is new news
		if(NewsUtils.isNewNews(DATE_FORMAT.format(article.getCreateDate()))) {
			setNew(true);
		}
		//get JournalStructure of news
		JournalStructure journalStructure = StructureUtil.getStructureById(article.getStructureId());
	...
	/**
	 * @return the urlDetailAudio
	 */
	public String getUrlDetailAudio() {
		return urlDetailAudio;
	}

	/**
	 * @param urlDetailAudio the urlDetailAudio to set
	 */
	public void setUrlDetailAudio(String urlDetailAudio) {
		this.urlDetailAudio = urlDetailAudio;
	}
