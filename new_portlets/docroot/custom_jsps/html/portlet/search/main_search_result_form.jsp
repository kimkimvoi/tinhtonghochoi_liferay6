...
if (assetRendererFactory != null) {
	classPK = GetterUtil.getLong(document.get(Field.ENTRY_CLASS_PK));
...
if (entryTitle != null && entrySummary != null) {
	...
	if(className.contains("JournalArticle")) {
		try {
			JournalArticleResource journalArticleResourceObj = JournalArticleResourceLocalServiceUtil.getJournalArticleResource(classPK);
			...
			String vers = String.valueOf(journalArticleObj.getVersion());
			String structsId = journalArticleObj.getStructureId();
			List<AssetCategory> categories = AssetCategoryLocalServiceUtil.getCategories(JournalArticle.class.getName(), journalArticleObj.getResourcePrimKey());
			String categoryId = "";
			String cateName = "";
			if (null != categories && categories.size() > 0) {
				categoryId = String.valueOf(categories.get(0).getCategoryId());
				cateName = categories.get(0).getName();
			} 
			long groupIdTheme = themeDisplay.getScopeGroupId();
			long layoutId = LayoutLocalServiceUtil.getFriendlyURLLayout(groupIdTheme, false, "/news").getPlid();
			
			PortletURL portletURL = PortletURLFactoryUtil.create(request, "newsdetail_WAR_bacninhportlet", layoutId, PortletRequest.RENDER_PHASE);
			try {
				portletURL.setWindowState(WindowState.NORMAL);
				portletURL.setPortletMode(PortletMode.VIEW);
				portletURL.setParameter("articleId", articleId);
				portletURL.setParameter("version", StringUtil.replace(String.valueOf(vers), StringPool.PERIOD, StringPool.DASH));
				portletURL.setParameter("structId", structsId);
				portletURL.setParameter("categoryId", String.valueOf(categoryId));
				portletURL.setParameter("cateName", cateName);
			} catch (Exception e) {
				e.printStackTrace();
			}
...
<% } else { %>
<span style="color: #C0C0C0; font-style: italic; text-decoration: line-through!important;"><liferay-ui:message key="portlet.search.journalarticle.notexist" /></span>	
<% } %>