...
String structureName = structure.getName(languageId, true);

Group group = themeDisplay.getScopeGroup();

if (group.isLayout()) {
	
	List<AssetVocabulary> lstAssetVoca = AssetVocabularyLocalServiceUtil.getGroupVocabularies(group.getParentGroupId(), false);
	for (AssetVocabulary assetVoca : lstAssetVoca) {
		if (structureName.equals(assetVoca.getTitle(languageId, true))) {
			vocabularies.add(assetVoca);		
		}
	}
	
}
else {
	List<AssetVocabulary> lstAssetVoca = AssetVocabularyLocalServiceUtil.getGroupVocabularies(scopeGroupId, false);
	for (AssetVocabulary assetVoca : lstAssetVoca) {
		if (structureName.equals(assetVoca.getTitle(languageId, true))) {
			vocabularies.add(assetVoca);		
		}
...
if (scopeGroupId != themeDisplay.getCompanyGroupId()) {
	List<AssetVocabulary> lstAssetVoca = AssetVocabularyLocalServiceUtil.getGroupVocabularies(themeDisplay.getCompanyGroupId(), false);
	for (AssetVocabulary assetVoca : lstAssetVoca) {
		if (structureName.equals(assetVoca.getTitle(languageId, true))) {
			vocabularies.add(assetVoca);		
		}
